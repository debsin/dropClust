...
call_louvain<-function(){
  ...
  print("Assiging Louvain Communities......................")
  system("rm output.graph",ignore.stderr = T)
  system(paste("sh louvain_script.sh", LOUVAIN_DIR,  "src_dst_lsh.csv", "output.graph",sep = " "))
  # K = readMM("lsh_nn.mtx")
  # KK = as.matrix(K)
  # 
  # gl = graph_from_adjacency_matrix(K,mode = "undirected")
  # mm = cluster_louvain(gl, weights = NULL)
  # write.table(mm$membership, file= "output.graph",row.names = T,col.names = F,quote = F)
}
...
ss_clustering<-function(ss_sel_genes, mincl = 20){
  d = dist(ss_sel_genes)
